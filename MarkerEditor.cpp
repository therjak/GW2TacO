...
TBOOL GW2MarkerEditor::IsMouseTransparent(CPoint &ClientSpacePoint,
    ...
    case WBM_COMMAND: {
      ...
      if (b->GetID() == _T( "starttrail" )) {
        ...
        GW2TrailDisplay *trails =
            (GW2TrailDisplay *)App->GetRoot()->FindChildByID(_T( "trail" ),
                                                             _T( "gw2Trails" ));
    ...
    case WBM_CONTEXTMESSAGE:
      if (Message.Data >= 0 && Message.Data < CategoryList.size()) {
        if (!ChangeDefault) {
          ...
          CWBLabel *type =
              (CWBLabel *)FindChildByID("defaultmarkertype", "label");
          if (type)
