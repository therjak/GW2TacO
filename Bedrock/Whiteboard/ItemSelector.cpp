...
CWBSelectableItem::CWBSelectableItem(std::string_view text, SELECTABLEID id,
                                     TBOOL selected) {
  Selected = selected;
  Text = text;
  ID = id;
...
std::string &CWBSelectableItem::GetText() { return Text; }

SELECTABLEID CWBSelectableItem::GetID() { return ID; }

TBOOL CWBSelectableItem::IsSelected() { return Selected; }

TBOOL CWBSelectableItem::IsColorSet() { return ColorSet; }

CColor CWBSelectableItem::GetColor() { return Color; }
...
void CWBSelectableItem::SetText(std::string_view text) { Text = text; }

void CWBSelectableItem::SetID(SELECTABLEID id) { ID = id; }

void CWBSelectableItem::Select(TBOOL selected) { Selected = selected; }
...
CWBItemSelector::CWBItemSelector(CWBItem *Parent, const CRect &Pos)
    : CWBItem() {
...
TBOOL CWBItemSelector::MessageProc(CWBMessage &Message) {
  switch (Message.GetMessage()) {
    case WBM_SELECTITEM:
      App->SendMessage(
          CWBMessage(App, WBM_ITEMSELECTED, GetGuid(), Message.Data));
      return true;
    default:
      break;
  }
...
SELECTABLEID CWBItemSelector::AddItem(std::string_view Text) {
  List += CWBSelectableItem(Text, wbItemIDCounter++, false);
  return List.Last().GetID();
}

SELECTABLEID CWBItemSelector::AddItem(std::string_view Text, SELECTABLEID ID) {
  List += CWBSelectableItem(Text, wbItemIDCounter++, false);
  List.Last().SetID(ID);
  return List.Last().GetID();
}
...
void CWBItemSelector::Flush() { List.Flush(); }
...
int32_t CWBItemSelector::NumItems() { return List.NumItems(); }
...
int32_t CWBItemSelector::GetCursorPosition() { return CursorPosition; }
...
void CWBItemSelector::SelectItemByIndex(
    int32_t Idx)  // single select implementation only
{
  // clear selection
  for (int32_t x = 0; x < List.NumItems(); x++) List[x].Select(false);

  CursorPosition = AnchorPosition = Idx;

  if (CursorPosition >= 0 && CursorPosition < List.NumItems()) {
    List[CursorPosition].Select(true);
    App->SendMessage(CWBMessage(App, WBM_SELECTITEM, GetGuid(),
                                List[CursorPosition].GetID()));
  }
