...
CCoreMaterialParameter::~CCoreMaterialParameter() { SAFEDELETE(Value); }
...
void CCoreMaterialParameter::SetType(COREMATERIALPARAMETERTYPE t) { Type = t; }

void CCoreMaterialParameter::SetScope(COREMATERIALPARAMETERSCOPE s) {
  Scope = s;
}

void CCoreMaterialParameter::SetName(CString &s) { Name = s; }

INLINE CString &CCoreMaterialParameter::GetName() { return Name; }

INLINE int32_t &CCoreMaterialParameter::GetDataSize() { return ValueSize; }

INLINE uint8_t *&CCoreMaterialParameter::GetData() { return Value; }
...
TBOOL CCoreMaterialParameter::Import(CXMLNode *Root) {
  ...
  if (!Root->GetChildCount(_T("Scope"))) {
    LOG_ERR("[core] Error importing Material Tech Input '%s': Scope missing",
            Name.GetPointer());
    return false;
  }
  ...
  if (!FindEnumByName(MaterialParameterScopeNames, Scope.GetPointer(),
                      (int32_t &)eScope)) {
    LOG_ERR(
        "[core] Error importing Material Tech Input '%s': Scope value '%s' "
        "unknown",
        Name.GetPointer(), Scope.GetPointer());
    return false;
  }

  if (!Root->GetChildCount(_T("Type"))) {
    LOG_ERR("[core] Error importing Material Tech Input '%s': Type missing",
            Name.GetPointer());
    return false;
  }
  ...
  if (!FindEnumByName(MaterialParameterTypeNames, Type.GetPointer(),
                      (int32_t &)eType)) {
    LOG_ERR(
        "[core] Error importing Material Tech Input '%s': Type value '%s' "
        "unknown",
        Name.GetPointer(), Type.GetPointer());
    return false;
  }
...
CCoreAtom *CCoreMaterialRenderPass::SpawnAtom(
    CORERENDERLAYERID TargetLayer, CCoreMesh *Mesh,
    CCoreConstantBuffer *Buffers[COREBUFFERSCOPECOUNT]) {
...
CCoreMaterialRenderPass::CCoreMaterialRenderPass(CCoreDevice *Device) {
  ...
  for (int32_t x = 0; x < COREBUFFERSCOPECOUNT; x++)
    for (int32_t y = 0; y < 3; y++) ConstantBufferIndices[y][x] = -1;
...
void CCoreMaterialRenderPass::SetName(CString &s) { Name = s; }
...
void CCoreMaterialRenderPass::Apply() {
  ...
  for (int32_t x = 0; x < SamplerStates.NumItems(); x++) {
    CDictionary<CORESAMPLER, CCoreSamplerState *>::KDPair *kp =
        SamplerStates.GetKDPair(x);
...
void CCoreMaterialRenderPass::Export(CXMLNode *Node) {
  ...
  Node->AddChild(_T("ShaderCode"))
      .SetText(GetVertexShader()->GetCode().GetPointer());
}
...
TBOOL CCoreMaterialRenderPass::Import(CXMLNode *Root) {
  ...
  for (int32_t x = 0; x < Root->GetChildCount(_T("SamplerState")); x++) {
    ...
    if (Success) {
      ...
      CORESAMPLER s;
      if (!FindEnumByName(SamplerNames, sampler.GetPointer(), (int32_t &)s))
        Success = false;
      if (Success) {
  ...
  if (Root->GetChildCount(_T("ShaderCode"))) {
    ...
    GetVertexShader()->SetCode(sh, CString(_T("vsmain")),
                               CString(_T("vs_5_0")));
    ...
    GetGeometryShader()->SetCode(sh, CString(_T("gsmain")),
                                 CString(_T("gs_5_0")));
...
// void CCoreMaterialRenderPass::SetConstantBufferIndices(CORESHADERTYPE s,
// CCoreShader *sh)
//{
//	if (!sh)
//	{
...
//	}
//
//	ConstantBufferIndices[s][CORE_BUFFER_SCENE] =
//sh->GetConstantBufferIndex("SceneData");
//	ConstantBufferIndices[s][CORE_BUFFER_LAYER] =
//sh->GetConstantBufferIndex("LayerData");
//	ConstantBufferIndices[s][CORE_BUFFER_OBJECT] =
//sh->GetConstantBufferIndex("ObjectData");
//	ConstantBufferIndices[s][CORE_BUFFER_GAMEDATA] =
//sh->GetConstantBufferIndex("GameData");
//	ConstantBufferIndices[s][CORE_BUFFER_TECHSTATIC] =
//sh->GetConstantBufferIndex("StaticTechData");
//	ConstantBufferIndices[s][CORE_BUFFER_TECHDYNAMIC] =
//sh->GetConstantBufferIndex("DynamicTechData");
//}
...
void CCoreMaterialTechnique::GatherData(COREBUFFERSCOPE Target,
                                        CCoreConstantBuffer *Buffer,
    ...
    switch (Target) {
      case CORE_BUFFER_TECHSTATIC:
        Gather = i->GetScope() != CORE_MATERIAL_ANIMATED &&
                 i->GetScope() != CORE_MATERIAL_EXTERNAL;
        break;
        ...
        if (i->GetScope() == CORE_MATERIAL_EXTERNAL) {
          ...
          if (Group) {
            if (!Group->GetData(i->GetName(), value))
              LOG_WARN(
                  "[core] Object Group material parameter %s not set, "
                  "defaulting to 0",
                  i->GetName().GetPointer());
          }
...
TBOOL CCoreMaterialTechnique::HasParameters(COREBUFFERSCOPE Scope) {
  for (int32_t x = 0; x < Inputs.NumItems(); x++) {
    ...
    switch (Scope) {
      case CORE_BUFFER_TECHSTATIC:
        if (i->GetScope() != CORE_MATERIAL_ANIMATED &&
            i->GetScope() != CORE_MATERIAL_EXTERNAL)
          return true;
        break;
      case CORE_BUFFER_TECHDYNAMIC:
...
void CCoreMaterialTechnique::CreateAtoms(
    CArray<CCoreAtom *> &Atoms, CCoreMesh *Mesh,
    CCoreConstantBuffer *Buffers[COREBUFFERSCOPECOUNT]) {
  for (int32_t x = 0; x < Passes.NumItems(); x++)
    Atoms += Passes[x]->SpawnAtom(TargetLayer, Mesh, Buffers);
}

void CCoreMaterialTechnique::AddParameter(CString &nme,
                                          COREMATERIALPARAMETERSCOPE Scope,
                                          COREMATERIALPARAMETERTYPE Type,
                                          int32_t ValueSize, void *Value) {
...
CString &CCoreMaterialTechnique::GetName() { return Name; }

void CCoreMaterialTechnique::SetName(CString &name) { Name = name; }
...
TBOOL CCoreMaterialTechnique::Import(CXMLNode *Root, CCoreDevice *Device) {
  ...
  if (!passcount)
    LOG_WARN(
        "[core] Material Technique '%s' has no renderpasses and will do "
        "nothing",
        Name.GetPointer());
    ...
    if (!p->Import(&Root->GetChild(_T("InputParameter"), x)))
      ...
      for (int32_t y = 0; y < Inputs.NumItems(); y++)
        if (Inputs[y]->GetName() == p->GetName()) {
          LOG_WARN("[core] Material Tech Input name '%s' not unique",
                   Name.GetPointer());
          break;
        }
      Inputs += p;
...
void CCoreMaterial::GatherData(
    CCoreDevice *Device, COREBUFFERSCOPE Scope, CCoreObjectGroup *Group,
    CDictionary<CCoreMaterialTechnique *, CCoreConstantBuffer *> *Buffers[3],
  ...
  for (int32_t x = 0; x < Techniques.NumItems(); x++) {
    ...
    if (!Buf)
      Buf = (*Buffers[TargetBuffer])[tech] = Device->CreateConstantBuffer();

    tech->GatherData(Scope, Buf, Group);
  }
}

void CCoreMaterial::CreateAtoms(
    CDictionaryEnumerable<CORERENDERLAYERID, CCoreRenderLayer *> &Layers,
    CCoreConstantBuffer *SceneBuffer, CCoreConstantBuffer *ObjectBuffer,
    CDictionary<CCoreMaterialTechnique *, CCoreConstantBuffer *> *Buffers[3],
    CArray<CCoreAtom *> &Atoms, CCoreMesh *Mesh) {
  for (int32_t x = 0; x < Techniques.NumItems(); x++) {
    CCoreRenderLayer *Layer =
        Layers.GetExisting(Techniques[x]->GetTargetLayer());
    if (!Layer) continue;
...
int32_t CCoreMaterial::GetTechCount() { return Techniques.NumItems(); }
