...
TBOOL CCoreDX11BlendState::Update() {
  ...
  if (res != S_OK) {
    _com_error err(res);
    LOG(LOG_ERROR, _T("[core] DirectX11 Blend state creation failed (%s)"),
        err.ErrorMessage());
    return false;
  }
...
TBOOL CCoreDX11DepthStencilState::Update() {
  ...
  if (res != S_OK) {
    _com_error err(res);
    LOG(LOG_ERROR,
        _T("[core] DirectX11 Depth Stencil state creation failed (%s)"),
        err.ErrorMessage());
    return false;
  }
...
TBOOL CCoreDX11RasterizerState::Update() {
  ...
  if (res != S_OK) {
    _com_error err(res);
    LOG(LOG_ERROR, _T("[core] DirectX11 Rasterizer state creation failed (%s)"),
        err.ErrorMessage());
    return false;
  }
...
TBOOL CCoreDX11SamplerState::Update() {
  ...
  if (res != S_OK) {
    _com_error err(res);
    LOG(LOG_ERROR, _T("[core] DirectX11 Sampler state creation failed (%s)"),
        err.ErrorMessage());
    return false;
  }
