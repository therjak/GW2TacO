...
class CCoreDevice {
 ...
 protected:
  ...
  virtual TBOOL InitAPI(const uint32_t hWnd, const TBOOL FullScreen,
  ...
  virtual TBOOL ApplyRenderState(const CORESAMPLER Sampler,
 ...
 public:
  ...
  virtual TBOOL Initialize(CCoreWindowHandler *Window,
  ...
  virtual void SetFullScreenMode(const TBOOL FullScreen, const int32_t xr,
                                 const int32_t yr) = 0;
  ...
  virtual std::unique_ptr<CCoreTexture2D> CreateTexture2D(
      ...
      const TBOOL RenderTarget = false) = 0;
  virtual std::unique_ptr<CCoreTexture2D> CreateTexture2D(
  ...
  virtual TBOOL Clear(const TBOOL clearPixels = true,
                      const TBOOL clearDepth = true,
