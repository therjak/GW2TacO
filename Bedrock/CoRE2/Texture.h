...
class CCoreTexture2D : public CCoreTexture {
 ...
 public:
  ...
  virtual TBOOL Create(const int32_t XRes, const int32_t YRes,
  ...
  virtual TBOOL CreateDepthBuffer(const int32_t XRes, const int32_t YRes,
                                  const int32_t MSCount) = 0;
  virtual TBOOL Lock(void **Result, int32_t &pitch) = 0;
  virtual TBOOL UnLock() = 0;

  virtual TBOOL Update(const uint8_t *Data, const int32_t XRes,
  ...
  virtual void ExportToImage(std::string_view Filename, TBOOL ClearAlpha,
