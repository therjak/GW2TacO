...
CQuaternion Slerp(const CQuaternion &v1, const CQuaternion &v2, const float t) {
  ...
  return (v1 * sinf(angle * (1 - t)) + v2 * multiplier * sinf(angle * t)) /
         sinf(angle);
}
...
void CQuaternion::Normalize() { *this = Normalized(); }

CQuaternion CQuaternion::Inverted() const { return CQuaternion(-x, -y, -z, s); }

void CQuaternion::Invert() { *this = Inverted(); }
...
CQuaternion CQuaternion::operator+() const { return *this; }
