...
class CDictionary {
 ...
 protected:
  ...
  virtual KDPair *AddNew(const KeyType &Key) {
    ...
    if (Fill >= HASHEXPANSIONTHRESHOLD) ResizeTable(TableSize * 2);
 ...
 public:
  ...
  ItemType &GetByKey(const KeyType &Key) { return (*this)[Key]; }

  ItemType GetExisting(
      const KeyType &Key)  // this can only be used on dictionaries where the
  ...
  virtual void Delete(const KeyType &Key) {
    ...
    float Fill = ItemCount / (float)TableSize;
    if (Fill < HASHCOLLAPSETHRESHOLD) ResizeTable(TableSize / 2);
  }
  ...
  void FreeAll() {
    for (int32_t x = NumItems() - 1; x >= 0; x--) FreeByIndex(x);
  }

  void FreeAllA() {
    for (int32_t x = NumItems() - 1; x >= 0; x--) FreeAByIndex(x);
  }
  ...
  bool HasKey(const KeyType &Key) const { return Find(Key) != nullptr; }

  virtual int32_t NumItems() const { return ItemCount; }
  ...
  virtual CDictionary<KeyType, ItemType> &operator+=(
      const CDictionary<KeyType, ItemType> &i) {
  ...
  CDictionary<KeyType, ItemType> operator+(
      const CDictionary<KeyType, ItemType> &d1) const {
  ...
  const CDictionary<KeyType, ItemType> &operator=(
      const CDictionary<KeyType, ItemType> &dict) {
...
template <typename KeyType, typename ItemType>
class CDictionaryEnumerable : public CDictionary<KeyType, ItemType> {
#define SORTSTACKSIZE (8 * sizeof(void *) - 2)
 ...
 public:
  ...
  void Delete(const KeyType &Key) override {
    ...
    if (p) IndexMap.erase(std::remove(IndexMap.begin(), IndexMap.end(), p));
  }
  ...
  virtual void SortByKey(KEYSORTCALLBACK SortCallback) {
    ...
    // qsort implementation - calling crt qsort isn't viable here due to
    // template hackery implementation taken from crt
    ...
    if (size <= 8)  // cutoff = 8
    {
      ...
      while (_hi > _lo) {
        ...
        for (uint32_t p = _lo + 1; p <= _hi; p++)
          if (SortCallback(IndexMap[p]->Key, IndexMap[max]->Key) > 0) max = p;
        swap(max, _hi);
        _hi--;
      }
    } else {
      ...
      if (SortCallback(IndexMap[lo]->Key, IndexMap[mid]->Key) > 0)
        swap(lo, mid);
      if (SortCallback(IndexMap[lo]->Key, IndexMap[hi]->Key) > 0) swap(lo, hi);
      if (SortCallback(IndexMap[mid]->Key, IndexMap[hi]->Key) > 0)
        swap(mid, hi);
      ...
      for (;;) {
        if (mid > loguy) do {
            loguy++;
                   ...
                   SortCallback(IndexMap[loguy]->Key, IndexMap[mid]->Key) <= 0);
        if (mid <= loguy) do {
            loguy++;
          } while (loguy <= hi &&
                   SortCallback(IndexMap[loguy]->Key, IndexMap[mid]->Key) <= 0);
        do {
          higuy--;
                 ...
                 SortCallback(IndexMap[higuy]->Key, IndexMap[mid]->Key) > 0);
  ...
  recurse:
    ...
    if (size <= 8)  // cutoff = 8
    {
      ...
      while (_hi > _lo) {
        uint32_t max = _lo;
        for (uint32_t p = _lo + 1; p <= _hi; p++)
          if (SortCallback(IndexMap[p]->Data, IndexMap[max]->Data) > 0) max = p;
        swap(max, _hi);
        _hi--;
      }
    } else {
      ...
      if (SortCallback(IndexMap[lo]->Data, IndexMap[mid]->Data) > 0)
        swap(lo, mid);
      if (SortCallback(IndexMap[lo]->Data, IndexMap[hi]->Data) > 0)
        swap(lo, hi);
      if (SortCallback(IndexMap[mid]->Data, IndexMap[hi]->Data) > 0)
        swap(mid, hi);
      ...
      for (;;) {
        if (mid > loguy) do {
            loguy++;
          } while (loguy < mid && SortCallback(IndexMap[loguy]->Data,
                                               IndexMap[mid]->Data) <= 0);
        if (mid <= loguy) do {
            loguy++;
          } while (loguy <= hi && SortCallback(IndexMap[loguy]->Data,
                                               IndexMap[mid]->Data) <= 0);
        do {
          higuy--;
        } while (higuy > mid &&
                 SortCallback(IndexMap[higuy]->Data, IndexMap[mid]->Data) > 0);
        if (higuy < loguy) break;
        swap(loguy, higuy);
        if (mid == higuy) mid = loguy;
      }
  ...
  virtual CDictionaryEnumerable<KeyType, ItemType> &operator+=(
      const CDictionaryEnumerable<KeyType, ItemType> &i) {
  ...
  CDictionaryEnumerable<KeyType, ItemType> operator+(
      const CDictionaryEnumerable<KeyType, ItemType> &d1) const {
  ...
  const CDictionaryEnumerable<KeyType, ItemType> &operator=(
      const CDictionaryEnumerable<KeyType, ItemType> &dict) {
    Flush();
    (*this) += dict;
    return *this;
  }
