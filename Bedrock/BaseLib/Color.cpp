...
const CColor Lerp(const CColor a, const CColor b, const float t) {
  CColor c;
  for (int32_t x = 0; x < 4; x++) c[x] = (uint8_t)((b[x] - a[x]) * t + a[x]);
  return c;
}
...
uint8_t& CColor::operator[](int32_t idx) { return ((uint8_t*)this)[idx]; }

CColor::operator uint8_t*() { return (uint8_t*)this; }

CColor::operator const uint8_t*() const { return (const uint8_t*)this; }
