...
CString::CString() { Initialize(); }

CString::~CString() { SAFEDELETEA(String); }
...
const CString &CString::Append(const CString &str, const uint32_t len) {
  ...
  if (lengthIncoming)
    memcpy(newString + lengthOriginal, str.String,
           lengthIncoming * sizeof(TCHAR));
...
const CString &CString::Append(const TS8 *str, const uint32_t len) {
...
#ifdef UNICODE
  lengthCalculated =
      MultiByteToWideChar(CP_UTF8, 0, str, lengthIncoming, NULL, 0);
#endif
  ...
  if (lengthCalculated) {
#ifdef UNICODE
    MultiByteToWideChar(CP_UTF8, 0, str, lengthIncoming,
                        newString + lengthOriginal, lengthCalculated);
#else
...
TBOOL CString::operator==(const CString &v) const {
  ...
  if (!String && !v.String) return true;
...
TBOOL CString::operator!=(const CString &v) const {
  if (Length() != v.Length()) return true;
  if (Length() == 0 && v.Length() == 0) return false;
  if (!String && !v.String) return false;
  if (!String) return true;
#ifdef HASHED_STRINGS
  if (Hash != v.Hash) return true;
#endif
  return _tcscmp(String, v.String) != 0;
}

TBOOL CString::operator!=(const TS8 *v) const { return *this != CString(v); }
...
TBOOL CString::operator<(const wchar_t *v) const { return *this < CString(v); }
...
TCHAR &CString::operator[](const int32_t idx) const { return String[idx]; }

//////////////////////////////////////////////////////////////////////////
//

uint32_t CString::Length() const { return LengthCached; }
...
void CString::DeleteRegion(int32_t pos, int32_t size) {
  ...
  memcpy(txt, String, sizeof(TCHAR) * pos);
  memcpy(txt + pos, String + pos + size,
         sizeof(TCHAR) * (Length() - pos - size));
  txt[Length() - size] = 0;
  *this = CString(txt);
  delete[] txt;
}
...
void CString::ToLower() {
  for (uint32_t x = 0; x < Length(); x++) String[x] = _totlower(String[x]);
  StringChanged();
}

void CString::ToUpper() {
  for (uint32_t x = 0; x < Length(); x++) String[x] = _totupper(String[x]);
  StringChanged();
}
...
// void CString::FormatVA(const wchar_t *format, va_list argList, CString
// &Result)
//{
//	if (!format)
//	{
...
uint32_t CString::GetSubstringCount(CString &v) const {
  uint32_t nCount = 0;
  uint32_t nStart = 0;
  while ((nStart = Find(v, nStart) + 1) != 0) nCount++;
  return nCount;
}
...
CString CString::Trimmed() const {
  int nStart = 0;
  int nEnd = 0;
  for (unsigned int i = 0; i < Length(); i++, nStart++)
    if (!_istspace(String[i])) break;
  for (int i = Length() - 1; i >= 0; i--, nEnd++)
    if (!_istspace(String[i])) break;
  if (nEnd)
...
void CString::DecodeBase64(uint8_t *&Data, int32_t &outsize) {
  ...
  while (in_len-- && (String[in_] != _T('=')) && is_base64(String[in_])) {
    ...
    if (i == 4) {
      ...
      for (i = 0; (i < 3); i++) ret.WriteByte(char_array_3[i]);
      i = 0;
    }
  }
  ...
  if (i) {
    ...
    for (j = 0; j < 4; j++) char_array_4[j] = find_base64(char_array_4[j]);
...
//		mov edx, DWORD ptr[ecx + eax * 4]; //TODO: make this architecture
//independent for 64bit builds 		push edx;
//
//		jmp loopstart;
//	loopend:
//
...
//}
//
// static int vsscanfw_proxy(const wchar_t *buffer, const wchar_t *format,
// va_list argPtr)
//{
//#ifndef _WIN64
//  size_t count = 0;
...
int32_t CString::Strncmp(const TCHAR *str, const TCHAR *str2,
                         const int32_t len) {
