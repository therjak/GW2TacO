...
class Matrix4x4 {
 public:
  constexpr Matrix4x4() : m{{{0}}} {}

  constexpr Matrix4x4(const Matrix4x4& mx) { m = mx.m; }
  constexpr Matrix4x4(float f11, float f12, float f13, float f14,  //
  ...
  [[nodiscard]] constexpr Vector4 Row(int32_t x) const {
    return Vector4(m[x & 3][0], m[x & 3][1], m[x & 3][2], m[x & 3][3]);
  }
  [[nodiscard]] constexpr Vector4 Col(int32_t x) const {
    return Vector4(m[0][x & 3], m[1][x & 3], m[2][x & 3], m[3][x & 3]);
  }
  constexpr Matrix4x4& operator*=(const Matrix4x4& mat) {
    return *this = (*this) * mat;
  }
  constexpr Matrix4x4& operator+=(const Matrix4x4& mat);
  constexpr Matrix4x4& operator-=(const Matrix4x4& mat);
  constexpr Matrix4x4& operator*=(const float f);
  constexpr Matrix4x4& operator/=(const float f);
  ...
  constexpr Matrix4x4 operator+(const Matrix4x4& mat) const;
  constexpr Matrix4x4 operator-(const Matrix4x4& mat) const;
  constexpr Matrix4x4 operator*(const float f) const;
  constexpr Matrix4x4 operator/(const float f) const;
  constexpr friend Matrix4x4 operator*(const float f, const Matrix4x4& mat);
  constexpr friend bool operator==(const Matrix4x4& lhs,
                                   const Matrix4x4& rhs) = default;

  constexpr void Transpose();
  [[nodiscard]] constexpr Matrix4x4 Transposed() const;
  constexpr Matrix4x4& SetIdentity();
  [[nodiscard]] constexpr Vector4 Apply(const Vector3& v) const;
  [[nodiscard]] constexpr Vector4 Apply(const Vector4& v) const;
  [[nodiscard]] constexpr float Determinant() const {
    return -Col(3) * Vector4::Cross(Col(0), Col(1), Col(2));
  }
  void Invert();
  [[nodiscard]] Matrix4x4 Inverted() const;
  void SetLookAtLH(const Vector3& Eye, const Vector3& Target,
                   const Vector3& Up);
  void SetLookAtRH(const Vector3& Eye, const Vector3& Target,
                   const Vector3& Up);
  ...
  void SetPerspectiveFovRH(const float fovy, const float aspect, const float zn,
                           const float zf);
  constexpr static Matrix4x4 Translation(const Vector3& v);
  constexpr static Matrix4x4 Scaling(const Vector3& v);
  static Matrix4x4 Rotation(const Vector3& Axis, const float Angle);

 private:
  std::array<std::array<float, 4>, 4> m = {{{0}}};
};

constexpr Matrix4x4::Matrix4x4(float f11, float f12, float f13, float f14,
                                 float f21, float f22, float f23, float f24,
                                 float f31, float f32, float f33, float f34,
                                 float f41, float f42, float f43, float f44) {
  m[0][0] = f11;
...
constexpr Vector4 Matrix4x4::Apply(const Vector4& v) const {
  Vector4 r;
...
constexpr Vector4 Matrix4x4::Apply(const Vector3& v) const {
  Vector4 r;
...
constexpr Matrix4x4& Matrix4x4::SetIdentity() {
...
constexpr void Matrix4x4::Transpose() { *this = Transposed(); }

constexpr Matrix4x4 Matrix4x4::Scaling(const Vector3& v) {
  Matrix4x4 mx;
  mx.SetIdentity();
  mx.m[0][0] = v.x;
  mx.m[1][1] = v.y;
...
constexpr Matrix4x4 Matrix4x4::Translation(const Vector3& v) {
  Matrix4x4 mx;
  mx.SetIdentity();
  mx.m[3][0] = v.x;
  mx.m[3][1] = v.y;
...
constexpr void Matrix4x4::SetOrthoRH(const float w, const float h,
...
constexpr void Matrix4x4::SetOrthoLH(const float w, const float h,
                                      const float zn, const float zf) {
  SetIdentity();
  m[0][0] = 2.0f / w;
  m[1][1] = 2.0f / h;
...
constexpr Matrix4x4& Matrix4x4::operator+=(const Matrix4x4& mat) {
  m[0][0] += mat.m[0][0];
  m[0][1] += mat.m[0][1];
  m[0][2] += mat.m[0][2];
  m[0][3] += mat.m[0][3];
...
constexpr Matrix4x4& Matrix4x4::operator*=(const float f) {
  m[0][0] *= f;
  m[0][1] *= f;
  m[0][2] *= f;
  m[0][3] *= f;
...
constexpr Matrix4x4 Matrix4x4::operator+() const { return *this; }

constexpr Matrix4x4 Matrix4x4::operator-() const {
  return {-m[0][0], -m[0][1], -m[0][2], -m[0][3], -m[1][0], -m[1][1],
          -m[1][2], -m[1][3], -m[2][0], -m[2][1], -m[2][2], -m[2][3],
          -m[3][0], -m[3][1], -m[3][2], -m[3][3]};
}

constexpr Matrix4x4 Matrix4x4::operator*(const Matrix4x4& mat) const {
  Matrix4x4 matT;
...
constexpr Matrix4x4 Matrix4x4::operator*(const float f) const {
...
constexpr Matrix4x4 operator*(const float f, const Matrix4x4& mat) {
