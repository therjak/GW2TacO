...
DX11VertexBuffer::DX11VertexBuffer(DX11Device* device) : VertexBuffer(device) {
...
bool DX11VertexBuffer::Create(const uint8_t* data, const uint32_t size) {
  ...
  const HRESULT result = d3d_device_->CreateBuffer(&buffer_desc, &vertex_data,
                                                   &vertex_buffer_handle_);
  if (result != S_OK) {
...
bool DX11VertexBuffer::Lock(void** result_ptr, const uint32_t offset,
  ...
  if (d3d_device_context_->Map(vertex_buffer_handle_, 0,
