...
// Snippet that creates an assertion function that works both in DEBUG & RELEASE
// mode. JSONXX_ASSERT(...) macro will redirect to this. assert() macro is kept
// untouched.
    ...
    if (ch == '\\') {
      ...
      switch (ch) {
        ...
        case 'u': {
          ...
          if (input.good() && (ss >> i)) value.push_back(static_cast<char>(i));
        } break;
        default:
          if (ch != delimiter) {
            value.push_back('\\');
...
class IOStateMasker {
 public:
  ...
  ~IOStateMasker() { stream.exceptions(mask); }
...
std::string escape_string(const std::string& input, const bool quote = false) {
  ...
  if (!once) {
    // The problem here is that, once is initializing at the end of job, but if
    // multithreaded application is starting this for the first time it will
    // jump into this part with several threads and cause double free or
    // corruptions. To prevent it, it is required to have mutex, to lock other
    // threads while only one of them is constructing the static map.
    mutex.lock();
    if (!once) {
      // base
      for (int i = 0; i < 256; ++i) {
...
std::string reformat(std::istream& input) {
  ...
  if (input.peek() == '{') {
    jsonxx::Object o;
    if (parse_object(input, o)) return o.json();
  } else if (input.peek() == '[') {
    jsonxx::Array a;
    if (parse_array(input, a)) return a.json();
  }
